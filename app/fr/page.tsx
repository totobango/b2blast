...
export default function HomeFR() {
  ...
  return (
    <>
        ...
        <div className="relative mx-auto max-w-6xl px-4 sm:px-6 lg:px-8 py-32 sm:py-40">
          <div className="max-w-3xl">
            <div className="hero-animate hero-delay-1 inline-flex items-center gap-2 rounded-full border border-brand/30 bg-brand/10 px-4 py-1.5 text-sm font-medium text-brand-light mb-8">
              ...
              B2B Uniquement — Français ↔ Anglais
            </div>
            ...
            <div className="hero-animate hero-delay-5 mt-10 flex flex-col sm:flex-row gap-4">
              <Link href="/fr/contact" className="btn-magnetic inline-flex items-center justify-center rounded-lg bg-brand px-7 py-4 text-base font-semibold text-white shadow-lg shadow-brand/30">
                {t.hero.cta}
              ...
              <Link href="/fr/contact#calendly" className="inline-flex items-center justify-center rounded-lg border border-gray-700 px-7 py-4 text-base font-semibold text-gray-300 hover:border-brand/50 hover:text-white transition-all">{t.hero.ctaSecondary}</Link>
            </div>
          </div>
      ...
      <section className="py-24 sm:py-32 bg-black">
        <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
          <ScrollReveal><div className="max-w-2xl mb-14"><h2 className="font-heading text-3xl sm:text-4xl font-bold text-white tracking-tight">{t.process.title}</h2><p className="mt-4 text-lg text-gray-400">{t.process.subtitle}</p></div></ScrollReveal>
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {t.process.steps.map((step, i) => (
              <ScrollReveal key={i} delay={i * 100}>
                <div className="relative rounded-2xl border border-white/10 bg-white/[0.03] p-6 h-full">
                  <div className="flex items-center gap-3 mb-4">
                    <span className="flex items-center justify-center w-8 h-8 rounded-lg bg-brand text-white text-sm font-bold">{step.step}</span>
                    <span className="text-xs text-brand-light font-medium uppercase tracking-wider">{step.duration}</span>
                  </div>
      ...
      {/* SERVICES */}
      <section className="py-24 sm:py-32 bg-black border-t border-white/5">
        <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
              ...
              <ScrollReveal key={platform.name} delay={i * 80}>
                <TiltCard className="rounded-xl border border-white/10 bg-white/[0.03] p-5 card-hover h-full" tiltAmount={5} glare={false}>
                  <div className="flex items-center gap-3 mb-2">
                    <Image src={`/images/platforms/${platform.icon}.png`} alt={`Logo ${platform.name}`} width={24} height={24} className="h-6 w-6 rounded object-contain" />
              ...
              <ScrollReveal key={pkg.name} delay={i * 120}>
                <TiltCard className={`relative rounded-2xl p-6 sm:p-8 h-full ${pkg.popular ? "bg-brand text-white ring-2 ring-brand-light shadow-xl shadow-brand/20" : "bg-white/[0.03] border border-white/10"}`} tiltAmount={3}>
                  <div className="flex flex-col h-full">
                    {pkg.popular && <span className="absolute -top-3 left-1/2 -translate-x-1/2 rounded-full bg-white px-4 py-1 text-xs font-semibold text-brand shadow-lg z-10">Le + populaire</span>}
                    ...
                    <Link href="/fr/contact" className={`btn-magnetic mt-8 block w-full rounded-lg py-3 text-center text-sm font-semibold transition-all ${pkg.popular ? "bg-white text-brand hover:bg-gray-100" : "bg-brand text-white hover:bg-brand-light"}`}>{pkg.cta}</Link>
                  </div>
                </TiltCard>
              </ScrollReveal>
            ))}
      ...
      <section className="py-24 sm:py-32 bg-white">
        <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
          <ScrollReveal><div className="max-w-2xl mb-14"><h2 className="font-heading text-3xl sm:text-4xl font-bold text-black tracking-tight">{t.faq.title}</h2></div></ScrollReveal>
          <div className="grid sm:grid-cols-2 gap-6">
            {t.faq.items.map((item, i) => (
              <ScrollReveal key={i} delay={i * 60}>
                <div className="rounded-2xl border border-gray-200 p-6 h-full">
                  <h3 className="font-heading text-base font-semibold text-black mb-3">{item.question}</h3>
            ...
            <div className="relative overflow-hidden rounded-3xl bg-black px-8 py-20 sm:px-16 sm:py-24 text-center">
              ...
              <div className="relative">
                ...
                <div className="mt-10 flex flex-col sm:flex-row items-center justify-center gap-4">
                  <Link href="/fr/contact" className="btn-magnetic inline-flex items-center rounded-lg bg-brand px-8 py-4 text-base font-semibold text-white shadow-lg shadow-brand/30">{t.cta.button}<svg className="ml-2 h-4 w-4" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="M13.5 4.5L21 12m0 0l-7.5 7.5M21 12H3" /></svg></Link>
                  <Link href="/fr/contact#calendly" className="inline-flex items-center rounded-lg border border-gray-700 px-8 py-4 text-base font-semibold text-gray-300 hover:border-brand/50 hover:text-white transition-all">{t.cta.buttonSecondary}</Link>
                </div>
              </div>
            </div>
          </ScrollReveal>
