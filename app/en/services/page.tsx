...
export const metadata: Metadata = { title: "Services", description: "B2B paid ads management on LinkedIn, Google, Microsoft & Reddit. Bilingual French ↔ English expertise. Packages from €2K/month." };
            ...
            {t.services.packages.map((pkg, i) => (
              <ScrollReveal key={pkg.name} delay={i * 120}>
                <div className={`relative rounded-2xl p-8 flex flex-col h-full ${pkg.popular ? "bg-brand text-white ring-1 ring-brand-light shadow-2xl shadow-brand/20" : "bg-surface-light border border-white/[0.06]"}`}>
                  {pkg.popular && <span className="absolute -top-3 left-1/2 -translate-x-1/2 rounded-full bg-cream px-4 py-1 text-xs font-bold text-brand shadow-lg z-10">Most Popular</span>}
                  ...
                  <Link href="/en/contact" className={`btn-magnetic mt-8 block w-full rounded-xl py-3.5 text-center text-sm font-semibold transition-all ${pkg.popular ? "bg-cream text-brand hover:bg-white" : "bg-brand text-white hover:bg-brand-light"}`}>{pkg.cta}</Link>
                </div>
              </ScrollReveal>
            ))}
          </div>
        ...
        <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8 text-center">
          <ScrollReveal>
            ...
            <div className="mt-10 flex flex-col sm:flex-row items-center justify-center gap-4">
              <Link href="/en/contact" className="btn-magnetic inline-flex items-center rounded-xl bg-brand px-8 py-4 text-base font-semibold text-white shadow-lg shadow-brand/25">{t.cta.button}</Link>
              <Link href="/en/contact#calendly" className="inline-flex items-center rounded-xl border border-white/10 px-8 py-4 text-base font-semibold text-gray-300 hover:border-brand/40 hover:text-white transition-all">{t.cta.buttonSecondary}</Link>
            </div>
          </ScrollReveal>
        </div>
      </section>
