          ...
          <div className="grid md:grid-cols-3 gap-6">
            {t.services.packages.map((pkg) => (
              <div key={pkg.name} className={`relative rounded-2xl p-6 sm:p-8 flex flex-col ${pkg.popular ? "bg-brand text-white ring-2 ring-brand-light shadow-xl shadow-brand/20" : "bg-white/[0.03] border border-white/10"}`}>
                {pkg.popular && <span className="absolute -top-3 left-1/2 -translate-x-1/2 rounded-full bg-white px-4 py-1 text-xs font-semibold text-brand z-10">Most Popular</span>}
                ...
                <Link href="/en/contact" className={`btn-magnetic mt-8 block w-full rounded-lg py-3 text-center text-sm font-semibold ${pkg.popular ? "bg-white text-brand hover:bg-gray-100" : "bg-brand text-white hover:bg-brand-light"}`}>{pkg.cta}</Link>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
          <div className="rounded-3xl bg-brand/10 border border-brand/20 px-8 py-16 sm:px-16 text-center">
            ...
            <div className="mt-8 flex flex-col sm:flex-row items-center justify-center gap-4">
              <Link href="/en/contact" className="btn-magnetic inline-flex items-center rounded-lg bg-brand px-8 py-4 text-base font-semibold text-white shadow-lg shadow-brand/30">{t.cta.button}</Link>
              <Link href="/en/contact#calendly" className="text-sm text-gray-400 hover:text-brand transition-colors">{t.cta.buttonSecondary}</Link>
            </div>
          </div>
        </div>
      </section>
